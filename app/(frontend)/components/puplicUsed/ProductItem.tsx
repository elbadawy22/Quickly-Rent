...
const ProductItem = ({ product }: { product: Products }) => {
  return (
    <Link href={`/admin/products/${product.id}`} key={product.id}>
      <div className="w-full overflow-hidden rounded-lg border border-zinc-200/80 bg-white shadow-sm">
        <div className="  flex flex-col items-center w-auto h-auto rounded-t-md overflow-hidden  ">
          {product.image?.length > 0 ? (
            <Image
              className="h-auto w-full max-w-[12rem] cursor-pointer overflow-hidden rounded-t-lg object-cover transition-transform duration-200 hover:scale-105"
              width={250}
              height={250}
